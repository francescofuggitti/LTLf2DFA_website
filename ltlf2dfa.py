            ...
            if params['formula']:
                try:
                    ...
                    if os.path.isfile("mona") and os.access("mona",
                        ...
                        subprocess.call('./mona -u -gw tool/automa.mona > images/'+automa_name+'.dot', shell=True)
                    else:
                        print('[ERROR] - MONA tool does not exist or it is not executable...')
                        exit()

                    ## POST-PROCESS MONA AUTOMATON ##
                    dot_handler = DotHandler('images/'+automa_name+'.dot')
                    dot_handler.modify_dot()
                    dot_handler.output_dot()

                    if os.path.isfile("mona"):  # check if automa exists
                        subprocess.call('dot -Tgif images/'+automa_name+'.dot -o images/'+automa_name+'.gif', shell=True)
                    else:
                        print('[ERROR] - MONA tool does not exist or it is not executable...')
                        exit()
